...
export default function InsecureDesignSandbox() {
  const [configVuln, setConfigVuln] = useState<any>(null);
  const [configSafe, setConfigSafe] = useState<any>(null);
  const [toggleRes, setToggleRes] = useState<any>(null);
  const [errorRes, setErrorRes] = useState<any>(null);
  const [featureName, setFeatureName] = useState<string>("");
  const [featureValue, setFeatureValue] = useState<boolean>(true);

  async function fetchVulnerableConfig() {
    const res = await fetch(getApiUrl(5100, "/vulnerable/config"));
    setConfigVuln(await res.json());
  }

  async function fetchSafeConfig() {
    const res = await fetch(getApiUrl(5100, "/safe/config"));
    setConfigSafe(await res.json());
  }

  async function toggleVuln() {
    const res = await fetch(getApiUrl(5100, "/vulnerable/toggle-feature"), {
      method: 'POST',
  ...
  async function toggleSafe() {
    const res = await fetch(getApiUrl(5100, "/safe/toggle-feature"), {
      method: 'POST',
      headers: {
  ...
  async function triggerError() {
    const res = await fetch(getApiUrl(5100, "/vulnerable/error"));
