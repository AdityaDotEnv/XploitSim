...
export default function CryptographicFailuresSandbox() {
  ...
  const [input, setInput] = useState<string>("my-secret-password");
  ...
  function addResult(title: string, body: any, explanation: string) {
    setResults((prev) => [
  ...
  async function postJSON(path: string, payload: any): Promise<ApiResponse> {
    setLoading(true);
    try {
